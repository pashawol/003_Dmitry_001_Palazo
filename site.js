$('body').on('submit', ".validate-form", function (e) {
  $xx = /^(\s*)?(\+)?([- _():=+]?\d[- _():=+]?){10,14}(\s*)?$/.test($(this).find('[name="phone"]').val());

  if ($(this).find('[name="name"]').val() == "") {
    $(this).find('.error-name').show().html('Представьтесь, пожалуйста.');
  ...
  if ($(this).find('[type="email"]').val() == "") {
    $(this).find('.error-email').show().html('Ошибка в email');
    event.preventDefault();
  } else {
    $(this).find('.error-email').hide();

  }
  if (!$(this).find('[name="chck"]').prop('checked')) { 
    $(this).find('.throw_error').show().html('Необходимо принять соглашение?');
    event.preventDefault();
  } else {
    $(this).find('.throw_error').hide();

  }

  // if (!$xx) {
  //   $(this).find('.errorx').empty();
  //   $('body #bthrow_error_phone').show(1000).html('Ошибка в номере');
...
$('#namey').focusout(function(){
  $('.form-m .throw_error').html('<input id="ats1195" type="hidden" name="ats1195" value="1">')
});
  ...
  // $('.bbwin').css('display','none')
   ...
   if(!b){
    ...
    $(document).click( function(event){
      if( $(event.target).closest(".product_param_value").length ) 
        return;
      $(".bbwin").fadeOut("slow");
