...
foreach ($this->files as $file) {
    $catx = $file[tableSchema]->name;
    $tabx= 1;
    if ($catx=='catalog_category_images'){$tabx= 2;}
    if(!isset($file->alt)){$filealt="";}else{$filealt=$file->alt;}
	echo CHtml::tag('li', array('id' => 'photo-' . $file->id, 'data-id' => $file->id), CHtml::link(CHtml::image($file->getFileUrl('s')), $file->getFileUrl('b'), array('target' => '_blank')).'<a href="#" title="Удалить" class="remove"><i class="icon-remove"></i></a>
        <input class="inpalt" data-tab="'.$tabx.'" data-id="'.$file->id.'"   name =alt-'.$file->id.' value="'.$filealt.'" size="30" />
        ');
}
